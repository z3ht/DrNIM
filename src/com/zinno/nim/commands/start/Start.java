...
import com.zinno.nim.commands.start.computer.ComputerGame;
...
public class Start implements SubCommand {
	...
	public void runCommand(CommandSender sender, Command cmd, String[] args) {
		...
		if(!player.hasPermission("nim.start")) {
			player.sendMessage(ChatColor.RED + "You don't have permission to start a game of NIM");
			return;
		}
		...
		switch(args.length) {
		case 1:
			ComputerGame sp = new ComputerGame();
		...
		case 2:
			...
			for(Player p : Bukkit.getOnlinePlayers()) {
				if(p.getName().equalsIgnoreCase(args[1])) {
					...
					player.sendMessage(ChatColor.GREEN + "The game invite has been sent");
					player.sendMessage(ChatColor.GREEN + "Type /NIM expire <Invited Player Name> to delete the invitation");
					player.sendMessage(ChatColor.GREEN + "It will automatically delete in 60 seconds");
					new BukkitRunnable() {
				        
			            @Override
			            public void run() {
			            	if(NIMvites.killNimInv(player.getName(), p.getName())) {
								player.sendMessage(ChatColor.GOLD + "The NIM invitation to " + p.getName() + " has expired");
								p.sendMessage(ChatColor.GOLD + "The NIM invitation from " + player.getName() + " has expired");
								return;
							}
			            }
